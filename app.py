...
def run_optimization(num_ind, num_gen):
    try:
        plant = SamplePlant()
...
def main():
    st.title("Steam Power Plant Optimization")
    st.write("This application optimizes the efficiency of a steam power plant.")
    
    num_ind = st.slider("Number of Individuals", min_value=10, max_value=100, value=50)
    num_gen = st.slider("Number of Generations", min_value=10, max_value=100, value=50)
    
    if st.button("Run Optimization"):
        st.write("Running optimization...")
        svg_file = run_optimization(num_ind, num_gen)
